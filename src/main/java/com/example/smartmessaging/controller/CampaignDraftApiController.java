...
public class CampaignDraftApiController {
    ...
    public ResponseEntity<Map<String, Object>> updateRecipients(
            ...
            @Valid @RequestBody com.example.smartmessaging.dto.request.UpdateRecipientsRequest body,
            @AuthenticationPrincipal CustomUserDetails userDetails) {

        List<com.example.smartmessaging.dto.request.RecipientItem> items = body.getItems();
        if (items == null || items.isEmpty()) {
            return ResponseEntity.ok(Map.of());
        }

        for (com.example.smartmessaging.dto.request.RecipientItem item : items) {
