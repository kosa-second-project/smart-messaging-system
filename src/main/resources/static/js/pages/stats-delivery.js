...
function drawGroupedBarChart(canvas, rows, options) {
    ...
    const padding = { top: 8, right: 16, bottom: 42, left: 46 };
    ...
    const min = options.yMin ?? 0;
    const max = options.yMax ?? Math.max.apply(null, values);
    const ticks = 4;

    clearCanvas(ctx, width, height);
    drawGrid(ctx, padding, chartWidth, chartHeight, min, max, ticks, options.yFormatter);

    const groupGap = 14;
    const groupWidth = chartWidth / rows.length;
    const barGap = 3;
    const barWidth = Math.max(4, Math.min(12, (groupWidth - groupGap - barGap * (options.keys.length - 1)) / options.keys.length));
    ...
    rows.forEach(function(row, rowIndex) {
        ...
        options.keys.forEach(function(key, keyIndex) {
            ...
            drawRoundedTopBar(ctx, x, y, barWidth, barHeight, 3, options.colors[keyIndex]);
        });
        drawText(ctx, row.label || row.stage, padding.left + rowIndex * groupWidth + groupWidth / 2, padding.top + chartHeight + 18, {
            align: "center",
            fill: "#6B6B80",
            size: 11
        });
...
function drawComposedTrendChart(canvas, rows) {
    ...
    const width = canvas.clientWidth;
    const height = canvas.clientHeight;
    const padding = { top: 8, right: 44, bottom: 42, left: 46 };
    const chartWidth = width - padding.left - padding.right;
    const chartHeight = height - padding.top - padding.bottom - 24;
    const max = Math.max.apply(null, rows.map(function(row) { return row.sends; }));
    const pointsSends = getLinePoints(rows, "sends", padding, chartWidth, chartHeight, 0, max);
    const pointsSuccess = getLinePoints(rows, "success", padding, chartWidth, chartHeight, 0, max);
    const rateRows = rows.map(function(row) {
        return { label: row.label, successRate: Number(((row.success / row.sends) * 100).toFixed(1)) };
    });
    const pointsRate = getLinePoints(rateRows, "successRate", padding, chartWidth, chartHeight, 90, 100);

    clearCanvas(ctx, width, height);
    drawGrid(ctx, padding, chartWidth, chartHeight, 0, max, 4, function(value) {
        return `${(value / 10000).toFixed(0)}만`;
    });
    drawRightAxis(ctx, padding, chartWidth, chartHeight);
    drawArea(ctx, pointsSends, padding.top + chartHeight, "#1843FA", 0.16);
    drawLine(ctx, pointsSends, "#1843FA", 2);
    drawLine(ctx, pointsSuccess, "#10B981", 2, [4, 3]);
    drawLine(ctx, pointsRate, "#F59E0B", 2.5);
    drawDots(ctx, pointsRate, "#F59E0B");
    ...
    drawLegend(ctx, ["발송", "성공", "성공률"], ["#1843FA", "#10B981", "#F59E0B"], padding.left, height - 14);
}
...
function drawRightAxis(ctx, padding, chartWidth, chartHeight) {
    [90, 95, 100].forEach(function(value) {
        const y = padding.top + chartHeight - ((value - 90) / 10) * chartHeight;
        drawText(ctx, `${value}%`, padding.left + chartWidth + 10, y + 4, {
            align: "left",
            fill: "#6B6B80",
            size: 11
        });
